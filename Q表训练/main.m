...
%     alpha = 0.1-0.1*i/test_num;
   ...
   % end
    ...
    if reward == 1
        ...
        if find_row(state_new_num,mytable_num) ~= 0 && ~isequal(state_num,state_new_num)
            %if isequal(state_num,
    ...
    if isnan(table_err(i))
        table_err(i) = 1e-30;
    end
    if sum(get_a_list(state_tmp(1),state_tmp(2),state_tmp(3),Q_table)) > 1
    end
end
